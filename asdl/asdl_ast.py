...
"""all done"""
"""we need to change the structure of AST. It should have two main components: the python ast and the language ast
    AST
    /\________
   /          \
language ast   python ast"""

class AbstractSyntaxTree(object):
    def __init__(self, production, realized_code_fields=None,realized_text_fields=None):
        ...
        if realized_text_fields:
            for field in realized_text_fields:
                self.add_child(field)
        if realized_code_fields:
            assert len(realized_code_fields) == len(self.production.fields)

            for field in realized_code_fields:
                self.add_child(field)
        else:
...
class RealizedTextField(Field):
    """wrapper of field realized with values for text. Field type is always gonna be string and cardinality most likely single."""
    def __init__(self, field, value=None, parent=None):
        super(RealizedTextField, self).__init__(field.name, field.type, field.cardinality)
