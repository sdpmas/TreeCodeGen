...
class VocabEntry(object):
    ...
    def add(self, word):
        if word not in self:
            ...
            return self[word]
    def add_from_entry(self,entry):
        i=0
        for id,word in entry.id2word.items():
            if word not in self:
                i+=1
                wid = self.word2id[word] = len(self)
                self.id2word[wid] = word
        print('no of words not already in vocab ',i)
    ...
    def from_corpus(corpus, size, freq_cutoff=0):
        ...
        vocab_entry.set_top(top_k_words)
        return vocab_entry
